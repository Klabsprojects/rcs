...
const IndentApproval = () => {
  ...
  // Calculate total persons from segments
  const calculateTotal = (indent) => {
    if (indent.segment && Array.isArray(indent.segment)) {
      return indent.segment.reduce((sum, seg) => sum + (seg.persons || 0), 0);
    }
    return 0;
  };
                        ...
                        <tr className="hover:bg-gray-50 transition-colors duration-150">
                          ...
                          <td className="border border-gray-300 px-4 py-2 text-center">
                            <button 
                              ...
                              className="px-3 py-1 bg-blue-600 text-white rounded text-sm hover:bg-blue-700 transition mr-2"
                            >
                              {expandedOrder === indent.id ? 'Hide' : 'View'}
                            </button>
                            <button className="px-3 py-1 bg-green-600 text-white rounded text-sm hover:bg-green-700 transition mr-1">
                              Approve
                            </button>
                              ...
                              <div className="bg-gray-50 p-6">
                                ...
                                {detailsLoading ? (
                                  ...
                                  <div className="space-y-4">
                                    ...
                                    {/* Segments */}
                                    {orderDetails.segment && orderDetails.segment.length > 0 && (
                                      <div className="bg-white rounded-lg p-4 border border-gray-200">
                                        <h3 className="text-lg font-medium text-gray-900 mb-3">Segments</h3>
                                        <div className="overflow-x-auto">
                                          <table className="w-full table-auto border-collapse border border-gray-300">
                                            <thead>
                                              <tr className="bg-gray-100">
                                                ...
                                                <th className="border border-gray-300 px-4 py-2 text-center">Persons</th>
                                              </tr>
                                            </thead>
                                            <tbody>
                                              {orderDetails.segment.map((segment, segIndex) => (
                                                <tr key={segment.id} className="hover:bg-gray-50">
                                                  <td className="border border-gray-300 px-4 py-2">{segIndex + 1}.</td>
                                                  <td className="border border-gray-300 px-4 py-2">{segment.category}</td>
                                                  <td className="border border-gray-300 px-4 py-2 text-center">{segment.persons}</td>
                                                </tr>
                                              ))}
                                            </tbody>
                                          </table>
